...
public class Answer {
            ...
            switch (msgCheckArray[0]) {
                case "/타이머":
                    if (msgCheckArray[1].equals("")) {
                        Listener.send(tmsg[2],"초를 입력해주세요.");
                    } else {
                        Listener.send(tmsg[2],tmsg[1] + "님의 명령으로 " + msgCheckArray[1].replace("초", "") + "초 타이머를 시작합니다.");
                ...
                case "/기능":
                case "/사용법":
                    Listener.send(tmsg[2],howToUse());
                    break;
                case "/낙낙":
                    Listener.send(tmsg[2],"낙지개구리는 낙낙하고 울지요");
                    break;
                case "/로또":
                    getLotto();
                    break;
                case "/쇼타임":
                    Listener.send(tmsg[2],UsaRoom());
                    break;
                case "/나무":
                    Listener.send(tmsg[2],"https://namu.wiki/w/" + msgCheckArray[1]);
                    break;
                case "/국어":
    ...
    public void getLotto() {
        LottoTask lottoTask = new LottoTask(tmsg[2]);
        lottoTask.execute();
    }

    public void getDebate(String msg, String cnt) {
        ExchangeTask exchangeTask = new ExchangeTask(tmsg[2],msg,cnt);
    ...
    public void Timer(String msg) {
        ...
        try {
            if (time <= 60) {
                ...
                Listener.send(tmsg[2],time + "초가 끝났습니다.");
            } else {
                Listener.send(tmsg[2],"너무 길어요");
            }
    ...
    public void dic(String msg, int code) {
        langTask langTask = new langTask(tmsg[2],msg, code);
        langTask.execute();
    }

    public void getNews(String msg, String secondMsg) {
        if (secondMsg.equals("순위")) {
            newsTask newsTask = new newsTask(tmsg[2],msg, true);
            newsTask.execute();
        } else{
            newsTask newsTask = new newsTask(tmsg[2],msg);
            newsTask.execute();
        }
    ...
    public void getDateCnt(String direction, String num) {
        ...
        if (direction.equals("/일뒤")) {
            ...
            Listener.send(tmsg[2],"오늘로부터 " + num + "일 후 : " + sdf.format(cal.getTime()));
        } else {
            cal.add(Calendar.DAY_OF_MONTH, -Integer.parseInt(num));
            Listener.send(tmsg[2],"오늘로부터 " + num + "일 전 : " + sdf.format(cal.getTime()));
        }
        ...
        if(howMuch.equals("/단차")) {
            ...
            if(egg>=1&&egg<=6) {
                ...
                if(next<6) {
                    ...
                    Listener.send(tmsg[2],sibal[egg]+"가 나왔어용");
                } else {
                    egg = random.nextInt(5);
                    Listener.send(tmsg[2],jang[egg]+"가 나왔어용");
                }
            } else {
                Listener.send(tmsg[2],"금알;");
            }
        } else {
            ...
            Listener.send(tmsg[2],builder.toString());
        }
